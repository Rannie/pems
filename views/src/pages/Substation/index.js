...
class Substation extends React.Component {
  ...
  fetchList = (params = {}) => {
    ...
    api.substation
      ...
      .then(res => {
        ...
        this.checkResolveAndNotify(substations);
        this.setState({
          loading: false,
          data: substations,
          pagination,
  ...
  checkResolveAndNotify = substations => {
  ...
  handleCreateClick = () => {
    ...
    history.push('/substation/edit');
  };

  handleSubstationClick = item => {
    const { history } = this.props;
    history.push({ pathname: '/substation/detail', state: item });
  };

  handleEditClick = (record, event) => {
    event.stopPropagation();
    const { history } = this.props;
    history.push({ pathname: '/substation/edit', state: record });
  };
  ...
  handleDeleteClick = (record, event) => {
    ...
    Modal.confirm({
      title: '注意',
      content: `你确定要删除 ${record.name} 这个变电站吗?`,
      okText: '删除',
      okType: 'danger',
      cancelText: '取消',
      onOk: () => {
        api.substation.delete({ data: { id: record._id } }).then(
          () => {
