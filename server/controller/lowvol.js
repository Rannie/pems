...
const { Lowvol, Substation, Transbox } = require('../model');

module.exports = class LowvolController {
  static async list(ctx) {
    ...
    const voltages = await Lowvol.find(query, null, opt);
    const count = await Lowvol.count(query);
  ...
  static async create(ctx) {
    ...
    const voltage = await Lowvol.findOne({ 'number': number });
    if (voltage) {
      ctx.body = ctx.util.refail(`${number} 调度号的高压室已经存在`);
      return;
    }

    const newVol = new Lowvol({
    ...
    if (subId) {
      ...
      substation.low_vols.push(newVol);
      substation.save();
    }
    if (transId) {
      const transbox = await Transbox.findOne({ _id: transId });
      transbox.low_vols.push(lowvol);
      transbox.save();
    }
  ...
  static async update(ctx) {
    ...
    if (voltage && voltage._id.toString() !== volId) {
      ctx.body = ctx.util.refail(`调度号 ${number} 的低压室已经存在`);
      return;
    }

    await Lowvol.findOneAndUpdate( { _id: volId }, {
      number, range, model
    }, null, (err) => {
      if (err) {
  ...
  static async delete(ctx) {
    ...
    await Lowvol.findByIdAndRemove(volId);
